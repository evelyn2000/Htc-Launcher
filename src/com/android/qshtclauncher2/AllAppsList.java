...
//    	addPackage(context, packageName, null);
//    }

    /**
     * Add the icons for the supplied apk called packageName.
     */
    //public void addPackage(Context context, String packageName, ArrayList<ShortcutInfo> launchFreqAppsItems) {
    public void addPackage(Context context, String packageName) {
        ...
        if (matches.size() > 0) {
            for (ResolveInfo info : matches) {
            	...
            	LauncherModel.addOrReplaceFreqAppInfo(appInfo);
                add(appInfo);
            }
        }
    }
...
//    	updatePackage(context, packageName, null);
//    }
    /**
     * Add and remove icons for this package which has been updated.
     */
    //public void updatePackage(Context context, String packageName, ArrayList<ShortcutInfo> launchFreqAppsItems) {
    public void updatePackage(Context context, String packageName){
        final List<ResolveInfo> matches = findActivitiesForPackage(context, packageName);
        if (LauncherLog.DEBUG) {
        ...
        if (matches.size() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (applicationInfo == null) {
                	applicationInfo = new ApplicationInfo(context.getPackageManager(), info, mIconCache, null);
                    add(applicationInfo);
                } else {
                ...
                LauncherModel.addOrReplaceFreqAppInfo(applicationInfo);
//            	if(launchFreqAppsItems != null && applicationInfo.componentName != null){
//                	for(ShortcutInfo sInfo : launchFreqAppsItems){
//                		if(applicationInfo.componentName.equals(sInfo.getComponentName())){
//                			applicationInfo.launchedFreq = sInfo.launchedFreq;
//                			applicationInfo.lastLaunchTime = sInfo.lastLaunchTime;
//                			break;
//                		}
//                	}
//            	}
